from re import S
import numpy as np
from env_unity import Action,  State, is_terminal_state, do_action
from e_greedy import e_greedy_policy as policy
from q_table import Q_table, getQ, setQ

def qlearn_static(alpha_learning_rate, epsilon_propability_of_best, gamma_discount_rate, num_of_episodes):
    for _ in range(num_of_episodes):
        ...
        while not is_terminal:
            ...
            max_q_value_of_available_actions = np.max([getQ(new_s, a) for a in Action.get_available_actions(new_s)])
...
qlearn_static(0.1, 0.1, 0.9, 100000)
# printQ_v2()